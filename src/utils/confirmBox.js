...
export function msgTipBox(vue, obj, cb = () => { }) {
  const {
    title = "",
    msg = "删除操作不可逆！",
    className = "el-icon-warning text-danger",
    showCancelButton = true,
    showClose = false,
    confirmButtonText = "确 定",
    cancelButtonClass = "el-button--info",
    customClass = "msg-danger-tip",
  } = obj || {};
  const html = `<div class='display-flex msg-danger-title'><i class='${className}'></i>提示</div><div class="msg-danger-content">${msg}</div>`
  msgBox(
    vue,
    {
      title,
...
export function customMessageBox(vue, obj, cb = () => { }) {
  ...
  const html =
    ...
    `<div>
		${msgTxt1
      ? `<span style="${msgTxtColor ? "color: " + msgTxtColor : ""
      }">${msgTxt1}</span>`
      : ""
    }
		<span>${msgTxt2}</span>
  ...
  if (tipObj) {
    ...
    if (thereDuplicates.length) {
      ...
      thereDuplicates.forEach((tKey) => {
        tipObj[tKey] &&
          msgList.push(
            `第${repeatKeyObj[tKey].repeatSerialNumArr.join("、")}行与第${repeatKeyObj[tKey].firstIndex + 1
            }行 ${tipObj[tKey] || "数据"}重复`
          );
      });
      messageListTip(vue, {
