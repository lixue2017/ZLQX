<template>
  <el-popover placement="bottom" width="700" v-model="showPopover" trigger="click" @show="handleShow">
    <div class="custom-conf-list-box" :class="[overseaLangObj.yu_yan_lang]">
      <div id="sortColumn" class="conf-list">
        <div v-for="(item, idx) in columnConfList" :key="idx" :class="[item.fixed && 'ignore-elements']"
          class="conf-col-item">
          <div class="base-flex-between">
            <div class="check-label">
              <el-checkbox v-model="item.showEnable" :disabled="!!item.fixed || item.request">{{ item.label
                }}</el-checkbox>
            </div>
            <span class="mandatory-check" :style="{ opacity: item.fixed || item.request ? 1 : 0 }">({{
    overseaLangObj.bt_required("必选") }})</span>
            <span class="mandatory-check" :style="{ opacity: item.fixed ? 1 : 0 }">({{ overseaLangObj.gd_lock("固定")
              }})</span>
            <div class="checkbox-icon" v-if="!item.fixed && showMoveBtn">
              <i :class="`iconfont shangyi ${firstIndex === idx && 'hide'}`" @click="handleClick('pre', idx)"></i>
              <i :class="`iconfont a-shangyi2 ${lastIndex === idx && 'hide'}`" @click="handleClick('next', idx)"></i>
            </div>
    ...
    item.widthKey === "minWidth"
      ...
      overseaLangObj.yu_yan_lang === "en-us" ? "WIDTH" : "宽度"
              ...
              <span class="width-inp">
                <el-input-number v-if="item.widthKey === 'minWidth'" v-model="item.minWidth" v-input-number-limit="0"
                  :controls="false" :max="390" :min="60" size="small"
                  :placeholder="overseaLangObj.please_input('请输入')" />
                <el-input-number v-else v-model="item.width" v-input-number-limit="0" :controls="false" :max="390"
                  :min="60" size="small" :placeholder="overseaLangObj.please_input('请输入')" />
              </span>
              <span>px</span>
            </div>
          </div>
      ...
      <div class="base-flex-between bottom-btn">
        <el-button size="small" v-throttle type="primary" plain @click="handleShow">重置</el-button>
        <div>
          <el-button size="small" v-throttle type="primary" @click="renewEvt">编辑</el-button>
          <el-button size="small" v-throttle type="primary" @click="submitForm">确定</el-button>
          <el-button size="small" plain @click="cancelEvt">取消</el-button>
        </div>
      </div>
    </div>

    <el-button slot="reference" size="small" v-throttle :round="columnConfObj.btnRound" class="col-btn"
      icon="iconfont lieshezhi">{{ basicLangObj.lsz_set_cols || "列设置" }}</el-button>
  </el-popover>
</template>
