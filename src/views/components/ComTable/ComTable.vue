<template>
  <div :class="`tableData ${!tableData.length && 'noData'}`">
    ...
    <el-table ref="myTable" :data="tableData" :height="tableHeight" :max-height="tableHeight || maxTableHeight" border
      :lazy="treeLazyLoad.lazy" :load="treeLazyLoad.load" :show-header="extraConfig && extraConfig.showHeader !== false"
      :class="extraConfig.tClassName" :header-cell-class-name="`table-header-cell ${extraConfig && extraConfig.stripe && 'striped'
    }`" :header-row-class-name="`table-header-row ${extraConfig && extraConfig.multiline && 'table-header-height'
    }`" :row-class-name="rowClassName" :cell-class-name="`el-table__cell ${extraConfig && extraConfig.stripe && 'striped'
    }`" v-loading="loading" :stripe="extraConfig && extraConfig.stripe === true ? extraConfig.stripe : false
    " :highlight-current-row="currentRow" @current-change="handleCurrentChange" @select="onTableSelect"
      @select-all="onTableSelectAll" @header-dragend="onMixinHeaderDragend" :row-key="(row) => getRowKey(row)"
      @sort-change="handleSort" :default-sort="customSort" :tree-props="treeProps"
      :default-expand-all="defaultExpandAll" :span-method="spanMethod" :show-summary="showSummary"
      ...
      <template v-for="(column, colIndex) in columnConfig">
        <el-table-column v-if="column.multiHeader" :label="column.label"
          :key="`${column.label}_${column.prop}_${colIndex}`" :header-align="column.align || 'center'"
          :align="column.align || 'center'" :class-name="`${column.className} ${disableMixinDragDrop(column)}`">
          <template v-for="(item, index) in column.list">
            <el-table-column :prop="item.prop" :label="item.label" :width="item.width" :minWidth="item.minWidth"
              :key="index" :header-align="item.align || 'center'" :align="item.align || 'center'"
              :sortable="item.sortable" :class-name="`${item.className} ${item.sortable && item.align === 'right' ? 'sortable-right' : ''
    }`" :formatter="item.render || ((cRow) => cellShowTxt(cRow, item))"
              :show-overflow-tooltip="item.showOverflowTooltip" v-if="!item.customRow">
              <template v-for="(eles, i) in item.list">
                <el-table-column :prop="eles.prop" :label="eles.label" :width="eles.width" :key="i"
                  :header-align="eles.align || 'center'" :align="eles.align || 'center'" :sortable="eles.sortable"
                  :class-name="`${eles.className} ${eles.sortable && eles.align === 'right'
      ? 'sortable-right'
      : ''
    }`" :formatter="eles.render" :show-overflow-tooltip="eles.showOverflowTooltip"
                  v-if="!eles.customRow">
                </el-table-column>
                <el-table-column v-else :prop="eles.prop" :label="eles.label" :width="eles.width" :key="i"
                  :header-align="eles.align || 'center'" :align="eles.align || 'center'" :sortable="eles.sortable"
                  :class-name="`${eles.className} ${eles.sortable && eles.align === 'right'
      ? 'sortable-right'
      : ''
    ...
    dicsList: slotDicsList(eles, scope),
            ...
            <el-table-column :prop="item.prop" :label="item.label" :width="item.width" :key="index"
              :header-align="item.align || 'center'" :align="item.align || 'center'" :sortable="item.sortable"
              :show-overflow-tooltip="item.showOverflowTooltip" :class-name="`${item.className} ${item.sortable && item.align === 'right' ? 'sortable-right' : ''
    }`" v-else>
              <template slot-scope="scope">
                <slot name="customRow" :slotProps="{
    prop: item.prop,
    ...
    dicsList: slotDicsList(item, scope),
        ...
        <el-table-column v-else-if="column.customRow" :align="column.align" :fixed="column.fixed" :prop="column.prop"
          :label="column.label" :width="column.width" :min-width="column.minWidth" :formatter="column.render"
          :show-overflow-tooltip="column.showOverflowTooltip" :key="`${column.prop}_${colIndex}`"
          :sortable="column.sortable" :class-name="`${column.className} ${column.sortable && column.align === 'right' ? 'sortable-right' : ''
    } ${disableMixinDragDrop(column)}`" :type="column.type">
          ...
          <template slot-scope="scope">
            <template v-if="column.editCellKeyName && !scope.row[column.editCellKeyName]
    ">
              <!-- 非单独处理时显示文本 -->
    ...
    colDictParams(column, scope.row) || "--"
      ...
      column.render || cellShowTxt(scope.row, column)
            ...
            <template v-else-if="column.isEditEasyCell &&
    column.editCellKeyName &&
    scope.row[column.editCellKeyName]
    ">
              <!-- 简易的单元格编辑 -->
              <EditEasyCell :formModel="scope.row" :fieldItem="{
    id: column.prop,
    ...
    dicsList: slotDicsList(column, scope),
        ...
        <el-table-column v-else :align="column.align" :fixed="column.fixed" :type="column.type" :index="indexRender"
          :prop="column.prop" :label="column.label" :width="column.width" :min-width="column.minWidth"
          :sortable="column.sortable" :selectable="selectionRow" :class-name="`${column.className} ${column.sortable && column.align === 'right' ? 'sortable-right' : ''
    } ${disableMixinDragDrop(column)}`" :show-overflow-tooltip="column.columnLink && column.columnCopyTxt
      ? false
      : column.showOverflowTooltip
    " :formatter="column.render || ((row) => cellShowTxt(row, column))" :key="`${column.prop}_${colIndex}`">
          <template v-if="column.customHeader" slot="header">
            <slot name="customHeader" :slotProps="{ column }"> </slot>
          </template>
          ...
          <template v-slot="{ row }" v-if="column.columnOperation ||
    column.basicType ||
    ...
    ">
            <template v-if="column.isDiffMark">
              <DiffMark :currentVal="row[column.currentValProp]" :verifyVal="row[column.verifyValProp]"
                :decimal="column.decimal" :isNumIntercept="column.isNumIntercept || false"
                :showCurrentVal="column.showCurrentVal || false" :showVerifyVal="column.showVerifyVal || false"
            ...
            <template v-if="column.columnLink">
              <template v-if="row[column.prop] || row[column.prop] === 0">
                ...
                <div v-else-if="column.columnCopyTxt" class="base-flex-align">
                  <div @click="handleView(row, column)" style="max-width: calc(100% - 16px)">
                    <TooltipOver class="primary-text-btn" :content="column.render ? column.render(row) : row[column.prop]
    " />
                  </div>
                  <!-- 显示复制图标 -->
                  <i style="margin-left: 2px" @click="handleCopyText(row[column.prop])"
                    class="el-icon-document-copy primary-text-color" />
                </div>
                <span v-else class="primary-text-btn" @click="handleView(row, column)">
                  {{ column.render ? column.render(row) : row[column.prop] }}
                </span>
              </template>
              <span v-else-if="column.noTxtLink" class="primary-text-btn" @click="handleView(row, column)">--</span>
              <span v-else>--</span>
            </template>
    ...
    column.render(row)
                  ...
                  <template v-else-if="column.decimal || column.decimal === 0">{{
    ...
    column.renderBottom(row)
                  ...
                  <template v-else-if="column.bottomDecimal || column.bottomDecimal === 0
    ">{{
      ...
      row[column.bottomProp] || "--"
            ...
            <template v-if="column.columnOperation">
              <div class="operation-btn-box">
                <template v-if="!rowOperationBtnListFn(row).length">--</template>

                <template v-else-if="column.columnMoreOperation">
                  <template v-for="(iBtn, i) in rowBtnList(row, column)">
                    <el-button :key="`btn_${i}`" size="medium" type="text" v-throttle :loading="iBtn.loading"
                      :style="{ color: iBtn.color }" class="operate-icon" @click="
    iBtn.handleClick
    ...
    iBtn.btnTextRender ? iBtn.btnTextRender(row) : iBtn.text
                  ...
                  <el-dropdown v-if="rowBtnList(row, column, true).length" class="drop-down-more">
                    <el-button size="medium" type="text">更多<i class="el-icon-arrow-down" /></el-button>
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item v-for="(drBtn, i) in rowBtnList(row, column, true)" @click.native="
    drBtn.handleClick
    ...
    drBtn.btnTextRender
      ...
      : drBtn.text
                ...
                <template v-else v-for="(btnItem, btnIndex) in rowOperationBtnListFn(row)">
                  <span :key="btnIndex" v-if="btnItem.isSelectBtns">
                    <slot name="isSelectBtns" :slotProps="{ item: btnItem, row }" />
                  </span>
                  <el-button :key="btnIndex" size="medium" type="text"
                    :disabled="btnItem.disabled ? btnItem.disabled : false" v-hasPermi="btnItem.hasPermission"
                    :loading="btnItem.loading" v-throttle :style="{ color: btnItem.color }" v-if="colShowBtnFn(btnItem, row, column) &&
    ...
    btnItem.btnTextRender
      ...
      : btnItem.text
            ...
            <div v-if="column.basicType" :class="[
    ...
    backgroundColor:
      ...
      `${colDictParams(column, row, true).color}1A`,
              ...
              <i v-if="column.dotColor" class="status-dot" :style="{
    ...
    column.render
      ...
      : colDictParams(column, row) || "--"
            ...
            <div v-if="column.TagArrConfig" class="txt-tagsColor" :style="{
    color:
      column.fontColor &&
    ...
    column.render
      ...
      : colDictParams(column.TagArrConfig[0], row) || "--"
            ...
            <div v-if="column.customDicsArr" :style="{
    color:
    ...
    column.render
      ...
      : customDicsName(column, row) || "--"
            ...
            <div v-if="column.columnTag">
              <!-- 自定义字典词 -->
              <el-tag :type="column.tagType" :class="column.tagClass" class="tagClass" :style="column.tagColor
      ? {
      ...
      : {}
    " size="small" v-if="row[column.prop] || row[column.prop] === 0">{{ row[column.prop] }}</el-tag>
              <span v-else>--</span>
            </div>
            <template v-if="column.fieldItem && column.isEdit">
              <FormSingle :formData="row" :fieldItem="{
    id: column.prop,
    placeholder: column.label,
    ...column.fieldItem,
  }" :dictionary="dictionary" @handleChange="handleChange" />
            </template>
    ...
    " -->
      <my-pagination :background="paginationConfig.background" :page="start" :limit="paginationConfig.limit" :total="total" :small="paginationConfig.small"
        :className="paginationConfig.className" :layout="paginationConfig.layout"
        :pageSizes="paginationConfig.pageSizes" @pagination="handlePagination" />
    </div>
    ...
    <!-- <el-backtop
      ...
      :bottom="55"
      style="
        background: linear-gradient(180deg, #68c5fc 0%, #0D6BE9 100%);
        color: #fff;
...
export default {
  ...
  methods: {
    ...
    rowClassName({ row, rowIndex }) {
      ...
      return `table-row ${rowSelection ? "row-selection" : rowStripe ? "row-stripe" : ""
        } ${isFirstTake ? "first-instorage" : ""} ${row.trClassName ? row.trClassName : ""
        }`;
    },
    // 排序
    handleSort(column) {
